 ...
 * AdminDashboard.tsx
 *
 * Inicio del COORDINADOR — Panel de Control PG1-PG2.
 *
 ...
 *
 * ── POR QUÉ ESTE FICHERO YA NO ES «EL» PANEL ────────────────────────────────
 *
 * Antes había un solo panel que se adaptaba con condicionales, y eso tenía un
 * coste que no se veía: los hooks de datos se ejecutan ANTES de cualquier
 * condicional del render, así que un evaluador —que no coordina a nadie— pedía
 * igualmente el resumen de tesis de toda la cohorte para luego no enseñarlo.
 *
 * Ahora `DashboardPage` decide el workspace y monta uno u otro. La consecuencia
 * es que las peticiones de coordinación solo existen si quien mira coordina.
 */
        ...
        <div className="dash-body">
                ...
                <div className="dash-flagship-grid">
                    <div className="dash-flagship-main">
                        ...
                        {/* Sin condicional de rol: a este panel solo llega
                            quien coordina. El evaluador tiene el suyo. */}
                        <WorkQueue
                    ...
                    <aside className="dash-aside" aria-label="Progreso académico">
                        {/*
                          * DEFECTO CORREGIDO: aquí solo había `progress ? tarjeta
                          * : esqueleto`. Como `progress` únicamente se calcula
                          * cuando el resumen llega con éxito, un fallo del
                          * endpoint dejaba este hueco girando con `aria-busy`
                          * PARA SIEMPRE: la zona de indicadores mostraba su error
                          * y ofrecía reintentar, mientras esta columna seguía
                          * anunciando a los lectores de pantalla que estaba
                          * cargando algo que ya no iba a llegar nunca.
                          *
                          * Toda carga termina en éxito, vacío o error. El error
                          * aquí es DISCRETO —no repite el mensaje ni duplica el
                          * botón de reintentar, que ya están arriba a la vista—
                          * pero cierra el ciclo y suelta el `aria-busy`.
                          */}
                        {summary.status === 'error' ? (
                            <Card padded className="dash-widget">
                                <span className="ui-kicker">Progreso académico</span>
                                <p className="dash-widget__unavailable">
                                    No disponible mientras no carguen los indicadores.
                                </p>
                            </Card>
                        ) : progress ? (
